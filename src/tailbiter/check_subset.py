...
class Checker(ast.NodeVisitor):
    ...
    def generic_visit(self, t):  # pragma: no cover
    ...
    def visit_ClassDef(self, t):
        assert self.scope_type == "module", "Nested classes are not supported %r" % (t,)
    ...
    ops1 = {ast.UAdd, ast.Invert, ast.USub, ast.Not}
    ...
    def visit_Attribute(self, t):
        ...
        if isinstance(t.ctx, ast.Load):
            ...
            assert False, "Only loads and stores are supported: %r" % (t,)
    ...
    def check_identifier(self, name):
        ...
        assert (
                len(name) <= 2 or not name.startswith("__") or name.endswith("__")
        ), "Mangled private names are not supported: %r" % (name,)


def has_negzero(num):
    return is_negzero(num) or (
            isinstance(num, complex) and (is_negzero(num.real) or is_negzero(num.imag))
    )
